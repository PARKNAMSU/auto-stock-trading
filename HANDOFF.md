...
- 이슈 #17의 실제 GitHub 서브 이슈로 MongoDB 컬렉션별 저장 작업 #18~#24를 생성했다.
  - #18 `trading_runs`: 거래 실행 이력
  - #19 `trade_decisions`: 매매 판단·전략 점수·위험검사 근거
  - #20 `orders`: 주문 요청 및 현재 상태
  - #21 `order_events`: 주문 상태 변경 이력
  - #22 `executions`: 실제 체결 및 거래 비용
  - #23 `portfolio_snapshots`: 계좌 자산·보유 종목 스냅샷
  - #24 `cash_movements`: 입출금·배당·환전 등 자금 변동
  - 각 티켓에 컬렉션 설명, 공통/개별 저장 항목, 콘솔 활용 목적, 작업 범위, 완료 조건 및 관련 이슈를 기입했다.
  - 기존 영속화 이슈 #12와 동일하게 `priority:P2`로 분류하고, GitHub에서 부모 연결과 전체 본문을 확인했다.
- 이슈 #1 상태를 GitHub 카드의 완료 조건과 현재 구현·테스트에 대조했다.
  - 거래대금 랭킹, 현재가, 종목 정보, ETF 여부, 시가총액, 평균 거래량, 수정 일봉·주봉, 가격 단위 정규화, MarketSnapshot 및 품질 표시가 구현되어 있다.
  - API 오류와 오래되거나 결측된 스냅샷 테스트가 있으며 `go test ./...`, `go vet ./...` 통과를 확인했다.
  - `internal/external/sector.PlaceholderResolver`를 기본 수집 경로에 연결했다. 공급자가 결정되지 않아 분류는 비워 두고 `MissingFields`에 계속 표시한다.
  - 실제 분류 공급자, 미국/한국 공통 분류체계, provenance 및 결측·실패 정책 구현을 서브 이슈 #25에 기록하고 부모 #1 아래 연결했다.
  - 저장소 작업 후 `go test ./...`, `go vet ./...`, `git diff --check`가 통과했다. #1에 placeholder 및 후속 #25를 설명하는 종료 댓글을 남기고 닫았다.

## 현재 상태

- 이슈 #17 MongoDB 기반 구현은 커밋 `3b6126c`로 저장되어 있다.
- `internal/external` 패키지 분리 및 MongoDB 설정 책임 이동은 커밋 `3de7dbc`로 저장되어 있다.
- MongoDB 컬렉션별 저장 항목과 구현 요구사항은 #18~#24에 초안으로 정리했다. 실제 저장 모델·인덱스·통계 집계는 아직 구현하지 않았다.
- 기존 #12에는 SQLite 구현이 명시되어 있어 MongoDB 기준 후속 이슈들과 문서 정합성 정리가 필요하다. 이번 작업에서 #12 본문은 변경하지 않았다.
- 이슈 #1은 닫혔다. 섹터 공급자 구현은 서브 이슈 #25로 분리되어 진행 대기 중이다. SEC SIC와 KRX 업종 체계가 다르며, 공통 GICS 데이터에는 공급·사용 조건 검토가 필요하다.
...
- GitHub 구현 이슈는 우선순위별로 다음과 같이 구성되어 있다.
  - P0: #2~#4, #17 — API, 계좌/포지션, 전략 점수, MongoDB 기반 구성 (#1 시장 데이터 수집은 완료)
  - P1: #5~#8 — 위험관리, 주문 생명주기, 포지션 크기, 매도 전략
  - P2: #9~#12, #18~#25 — 페이퍼 트레이딩, 스케줄러, 백테스트, 저장소, 컬렉션별 영속화 및 섹터 분류
  - P3: #13~#16 — 실거래 보호장치, 테스트 확대, 설정, 관측성

## 권장 다음 작업

1. GitHub의 열린 `priority:P0` 이슈와 선행관계를 확인한다.
2. #18~#24 저장 항목과 선행관계를 검토하고 #12의 SQLite 명시와 MongoDB 작업 범위를 정리한다. 현재 dry-run에서 가능한 #18 실행 이력, #19 판단 근거, #20 주문 저장부터 진행할 수 있다.
3. #25의 섹터 공급자와 미국/한국 공통 분류 체계 및 라이선스/커버리지 기준을 결정한다.
4. 수집된 가격 이력에 실제 전략 점수를 계산하는 이슈 #4를 진행한다.
